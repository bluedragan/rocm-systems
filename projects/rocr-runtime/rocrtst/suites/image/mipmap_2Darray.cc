...
//
// MipmapDataIntegrity2DTest - Data validation test
// Tests that data written to each mip level is correctly isolated and readable
//
void Mipmap2DArrayTest::MipmapDataIntegrity2DTest(void) {
    if (!image_ext_supported) {
        std::cout <<
        "  Image extension not supported, skipping MipmapDataIntegrity2DTest" << std::endl;
        return;
    }

    std::cout << "Subtest: MipmapDataIntegrity2DTest" << std::endl;
    std::cout << "  This test validates data isolation between 2D mipmap levels" << std::endl;
    std::cout << "  by writing unique patterns to each level and reading them back" << std::endl;

    hsa_status_t err;

    // Compute full mipmap chain levels
    uint32_t max_dim = std::max(mipmap_desc_.width, mipmap_desc_.height);
    uint32_t max_levels = 1 + (uint32_t)std::floor(std::log2((double)max_dim));
    ...
    for (const auto& gpu_agent : agent_list) {
        ...
        for (uint32_t level = 0; level < num_mipmap_levels_; level++) {
            ...
            size_t level_size = level_width * level_height * pixel_size;

            // Allocate host buffer and fill with unique pattern
            std::vector<uint8_t> write_data(level_size);
            for (size_t i = 0; i < level_size; i++) {
                // Create level-dependent pattern: (level * 17 + i) % 256
                write_data[i] = static_cast<uint8_t>((level * 17 + i) % 256);
            }

            // Define region to write
        ...
        std::cout << "  MipmapDataIntegrity2DTest: PASSED for agent" << std::endl;
    }
}

//
// MipmapSampling2DTest - GPU sampling test
// Tests that GPU can correctly sample from mipmap at different LOD levels
//
void Mipmap2DArrayTest::MipmapSampling2DTest(void) {
    if (!image_ext_supported) {
        std::cout <<
        "  Image extension not supported, skipping MipmapSampling2DTest" << std::endl;
        return;
    }

    std::cout << "Subtest: MipmapSampling2DTest" << std::endl;
    std::cout << "  This test validates GPU-side texture sampling from mipmaps" << std::endl;
    std::cout << "  using explicit LOD specification (similar to tex2DLod)" << std::endl;

    hsa_status_t err;

    // Compute full mipmap chain
    uint32_t max_dim = mipmap_desc_.width > mipmap_desc_.height ?
                       mipmap_desc_.width : mipmap_desc_.height;
    uint32_t max_levels = 1 + (uint32_t)std::floor(std::log2((double)max_dim));
    num_mipmap_levels_ = max_levels;
    ...
    for (const auto& gpu_agent : agent_list) {
        ...
        std::cout << "  Testing GPU sampling across " << num_mipmap_levels_ << " levels" << std::endl;
        ...
        for (uint32_t level = 0; level < num_mipmap_levels_; level++) {
            ...
            std::cout << "    Level " << level << ": API access verified" << std::endl;
        }
        ...
        std::cout << "  MipmapSampling2DTest: API validation PASSED for agent" << std::endl;
    }
}

//
// MipmapErrorHandling2DTest - Error handling test
// Tests that API properly handles invalid parameters and error conditions
// Validates robustness of mipmap implementation
//
void Mipmap2DArrayTest::MipmapErrorHandling2DTest(void) {
    if (!image_ext_supported) {
        std::cout <<
        "  Image extension not supported, skipping MipmapErrorHandling2DTest" << std::endl;
        return;
    }

    std::cout << "Subtest: MipmapErrorHandling2DTest" << std::endl;
    std::cout << "  This test validates error handling for invalid parameters" << std::endl;
    ...
    for (const auto& gpu_agent : agent_list) {
        std::cout << "  Testing error handling on GPU agent" << std::endl;
        ...
        // Test 1: Invalid mip level (at boundary)
        {
            ...
            // Try to get level == num_levels (off-by-one boundary)
            hsa_ext_image_t level_image;
            err = hsa_amd_mipmap_array_get_level(gpu_agent, &test_image_,
            ...
            std::cout << "    Test 1 - Boundary level (level == num_levels): PASSED" << std::endl;
        ...
        // Test 2: Invalid mip level (way out of range)
        {
            ...
            std::cout << "    Test 2 - Out-of-range level (999): PASSED" << std::endl;
        ...
        // Test 5: Null output pointer
        {
            err = hsa_amd_mipmap_array_create(gpu_agent, &mipmap_desc_, nullptr,
                HSA_ACCESS_PERMISSION_RW, 4, nullptr);

            ASSERT_NE(HSA_STATUS_SUCCESS, err)
                << "Test 5 FAILED: Should reject null output pointer";
            std::cout << "    Test 5 - Null output pointer: PASSED" << std::endl;
        }

        // Test 6: Excessive mip levels (more than mathematically possible)
        {
            uint32_t max_dim = std::max(mipmap_desc_.width, mipmap_desc_.height);
            ...
            ASSERT_NE(HSA_STATUS_SUCCESS, err)
                << "Test 6 FAILED: Should reject excessive mip levels ("
                << excessive_levels << " > " << max_possible << ")";
            std::cout << "    Test 6 - Excessive mip levels: PASSED" << std::endl;
        }

        // Test 7: Invalid descriptor (zero width)
        {
            ...
            ASSERT_NE(HSA_STATUS_SUCCESS, err)
                << "Test 7 FAILED: Should reject zero width";
            std::cout << "    Test 7 - Zero width: PASSED" << std::endl;
        }

        // Test 8: Invalid descriptor (zero height for 2D)
        {
            hsa_ext_image_descriptor_t invalid_desc = mipmap_desc_;
            ...
            ASSERT_NE(HSA_STATUS_SUCCESS, err)
                << "Test 8 FAILED: Should reject zero height for 2D";
            std::cout << "    Test 8 - Zero height (2D): PASSED" << std::endl;
        }

        // Test 9: Get level from null mipmap handle
        {
            hsa_ext_image_t null_mipmap;
            null_mipmap.handle = 0;
            hsa_ext_image_t level_image;

            err = hsa_amd_mipmap_array_get_level(gpu_agent, &null_mipmap, 0, &level_image);

            ASSERT_NE(HSA_STATUS_SUCCESS, err)
                << "Test 9 FAILED: Should reject null mipmap handle";
            std::cout << "    Test 9 - Null mipmap handle: PASSED" << std::endl;
        }

        // Test 10: Destroy null mipmap
        {
            hsa_ext_image_t null_mipmap;
            null_mipmap.handle = 0;

            err = hsa_amd_mipmap_array_destroy(&null_mipmap);

            ASSERT_NE(HSA_STATUS_SUCCESS, err)
                << "Test 10 FAILED: Should reject destroying null mipmap";
            std::cout << "    Test 10 - Destroy null mipmap: PASSED" << std::endl;
        }

        std::cout << "  MipmapErrorHandling2DTest: All 10 tests PASSED for agent" << std::endl;
    }
}
